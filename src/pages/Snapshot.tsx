...
import { Save, History, BarChart3, BookOpen, ChevronRight, MessageCircle, X, Bookmark } from 'lucide-react';
import {
...
export default function Snapshot() {
  ...
  const [ratings, setRatings] = useState<Record<string, SnapshotRating>>(() => {
    ...
    // Initialize from all possible categories across all types
  ...
  const previousRatings = useMemo(() => {
    ...
    MOCK_SNAPSHOTS[1].ratings.forEach((r) => {
      prev[r.categoryId] = r;
    });
    return prev;
  }, []);
        ...
        <CardHeader className="pb-2">
          <div className="flex items-start justify-between">
            <div className="flex-1 min-w-0">
              <CardTitle className="text-base font-heading leading-tight">{cat.name}</CardTitle>
              <div className="flex items-center gap-1 mt-1">
                <BookOpen className="h-3 w-3 text-muted-foreground shrink-0" />
                <p className="text-xs font-body text-muted-foreground italic truncate">{cat.scriptureRef}</p>
              </div>
              {cat.description && (
                <p className="text-xs font-body text-muted-foreground mt-1.5 line-clamp-2">{cat.description}</p>
              )}
            </div>
            <div className="text-right ml-3 shrink-0">
              <span className={`text-3xl font-heading font-bold ${getScoreColor(score)}`}>{score}</span>
              <p className={`text-xs font-body font-semibold ${getScoreColor(score)}`}>{getScoreLabel(score)}</p>
              {trend !== 0 && (
                <p className={`text-xs font-body font-bold ${trend > 0 ? 'text-score-high' : 'text-score-low'}`}>
        ...
        <CardContent className="space-y-3 pt-0">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label className="text-xs font-body font-semibold text-muted-foreground uppercase tracking-wide">Your Rating</Label>
              <span className="text-xs font-body text-muted-foreground">1 – 10 scale</span>
            </div>
            <Slider
              value={[score]}
            ...
            <div className="flex justify-between text-[10px] font-body text-muted-foreground">
              <span>1</span><span>5 avg</span><span>10 best ever</span>
            </div>
          </div>
          ...
          {cat.hasSpouseRating && (
            <div className="space-y-2 pt-2 border-t border-dashed">
              <div className="flex items-center justify-between">
                <Label className="text-xs font-body font-semibold text-muted-foreground">Spouse</Label>
                <span className={`text-lg font-heading font-bold ${getScoreColor(rating?.spouseScore ?? 5)}`}>
          ...
          {cat.hasChildRating && (
            <div className="space-y-2 pt-2 border-t border-dashed">
              <div className="flex items-center justify-between">
                <Label className="text-xs font-body font-semibold text-muted-foreground">Child</Label>
                <span className={`text-lg font-heading font-bold ${getScoreColor(rating?.childScore ?? 5)}`}>
                  {rating?.childScore ?? 5}
                </span>
              </div>
              <Slider
          ...
          {annotations[cat.id] ? (
            <div className="pt-2 border-t">
              ...
              <p className="text-xs font-body text-muted-foreground italic">{annotations[cat.id]}</p>
            </div>
          ) : (
            <button
              onClick={(e) => {
                e.stopPropagation();
                const note = prompt('Add a life event or note for this category (e.g., "Got promoted", "Lost a loved one", "Started new workout routine"):');
                if (note) setAnnotations((prev) => ({ ...prev, [cat.id]: note }));
              }}
              className="text-xs font-body text-muted-foreground hover:text-secondary transition-colors flex items-center gap-1 pt-1"
            >
    ...
    return (
      <div>
        <div className="flex items-center gap-2 mb-4">
          <div className={`h-7 w-1 rounded-full ${colorClass}`} />
          ...
          <span className="text-xs font-body text-muted-foreground ml-auto">
            Avg: {(cats.reduce((s, c) => s + (ratings[c.id]?.score ?? 5), 0) / cats.length).toFixed(1)}
          </span>
        </div>
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm font-body text-muted-foreground mt-1 max-w-xl">
              Rate your last 30 days honestly. Take 20–30 minutes to pause, pray, and reflect. Your Snapshot Companion will guide you.
            </p>
          </div>
          <div className="flex items-center gap-3">
            <Select value={snapshotType} onValueChange={(v) => setSnapshotType(v as SnapshotType)}>
              <SelectTrigger className="w-[200px] font-body text-sm h-9">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="member">Member Snapshot™</SelectItem>
                <SelectItem value="leader">Leader Snapshot™</SelectItem>
                <SelectItem value="advisor">Advisor Snapshot™</SelectItem>
                <SelectItem value="nonprofit">Nonprofit Snapshot™</SelectItem>
              </SelectContent>
            </Select>
            <Button
              variant={showCompanion ? 'default' : 'outline'}
              size="sm"
              className="gap-1.5 font-body text-sm h-9"
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div className="flex items-center gap-4">
                <div className="text-center">
                  <p className="text-3xl font-heading font-bold text-secondary">{avgScore}</p>
                  <p className="text-xs font-body text-muted-foreground">Overall Avg</p>
                </div>
                  ...
                  {categories.map((cat) => {
                    ...
                    return (
                      <button
                        key={cat.id}
                        onClick={() => handleCategoryFocus(cat)}
                        className={`text-center transition-all hover:scale-110 ${activeCategory?.id === cat.id ? 'scale-110' : ''}`}
                        title={cat.name}
                      >
                        <p className={`text-lg font-heading font-bold ${getScoreColor(s)}`}>{s}</p>
                        <p className="text-[9px] font-body text-muted-foreground max-w-[60px] truncate">{cat.name}</p>
                      </button>
                    );
                  })}
                </div>
              </div>
              <Button size="sm" onClick={handleSave} className="font-heading font-semibold h-9 px-5 gap-1.5">
        ...
        <Tabs defaultValue="snapshot" className="space-y-5">
          <TabsList className="font-body">
            <TabsTrigger value="snapshot" className="gap-1.5 font-body font-semibold text-sm">
              <Save className="h-4 w-4" /> Snapshot
            </TabsTrigger>
            <TabsTrigger value="history" className="gap-1.5 font-body font-semibold text-sm">
              <History className="h-4 w-4" /> History
            </TabsTrigger>
            <TabsTrigger value="radar" className="gap-1.5 font-body font-semibold text-sm">
              <BarChart3 className="h-4 w-4" /> Overview
            </TabsTrigger>
          </TabsList>

          <TabsContent value="snapshot">
            <div className={`grid gap-6 ${showCompanion ? 'lg:grid-cols-[1fr_380px]' : 'grid-cols-1'}`}>
              {/* Left: Scorecard */}
              <div className="space-y-8">
                ...
                <Card>
                  <CardContent className="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label className="text-sm font-heading font-semibold">My Purpose Statement</Label>
                      <Textarea
                        ...
                        className="text-sm font-body min-h-[80px] resize-none"
                        onFocus={() => setActiveCategory(null)}
                      />
                    </div>
                    <div className="space-y-2">
                      <Label className="text-sm font-heading font-semibold">My Goal for This Quarter</Label>
                      <Textarea
                        value={quarterlyGoal}
                ...
                <Card>
                  <CardHeader className="pb-3">
                    <CardTitle className="text-base font-heading">Major Issue for Discussion / Major Prayer Request</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <Textarea
                      value={majorIssue}
                      ...
                      className="text-sm font-body min-h-[100px] resize-none"
                    />
                  </CardContent>
                </Card>

                <div className="flex justify-end pb-8">
                  <Button size="lg" onClick={handleSave} className="font-heading font-semibold px-8 h-11 gap-2">
            ...
            {showCompanion && (
              <div className="lg:hidden fixed bottom-4 right-4 z-50">
                ...
                <div
                  id="mobile-companion"
                  className="hidden absolute bottom-16 right-0 w-[340px] h-[500px] shadow-2xl rounded-lg overflow-hidden"
                >
          ...
          <TabsContent value="history">
            <Card>
              <CardHeader>
                ...
                <p className="text-sm font-body text-muted-foreground">Track your growth across months. Look for patterns and celebrate progress.</p>
              </CardHeader>
              <CardContent>
          ...
          <TabsContent value="radar">
            <Card>
              <CardHeader>
                ...
                <p className="text-sm font-body text-muted-foreground">A bird's-eye view of where you stand across all categories.</p>
              </CardHeader>
              <CardContent>
